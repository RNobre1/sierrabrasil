...
14. **`const`/`let` com useCallback NAO sofrem hoisting.** Declarar `const fetchFoo = useCallback(...)` DEPOIS de um `useEffect` que o referencia causa `ReferenceError` em producao (minificador reordena). Sempre declarar useCallbacks ANTES dos useEffects que os usam. Diferente de `function` declarations que sofrem hoisting.
15. **`import *` de libs grandes quebra builds de producao.** `import * as X from "recharts"` ou `"lucide-react"` causa "can't access lexical declaration before initialization" no esbuild. Usar imports nomeados explicitos ou dynamic `import()`. Libs Radix UI sao pequenas e seguras com `import *`.
16. **RLS de admin precisa cobrir TODAS as tabelas que o admin pode acessar durante impersonation.** Sem policies explicitas, o admin nao consegue ler/editar dados do tenant impersonado. Tabelas necessarias: attendants, knowledge_base, whatsapp_instances, conversations, messages, agent_faqs, agent_leads, agent_memories, notifications.
