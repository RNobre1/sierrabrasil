...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch attendant config from DB to build system prompt
    let systemPrompt = "Você é um atendente virtual amigável e profissional. Responda em português brasileiro, de forma clara e concisa.";
    ...
    if (attendantId) {
      ...
      const { data: attendant } = await supabase
        .from("attendants")
        .select("name, persona, instructions")
        .eq("id", attendantId)
        .single();
...
## REGRAS INVIOLÁVEIS
1. NUNCA invente informações. Se não sabe, diga que vai verificar.
