...
function Particles({ count = 40 }: { count?: number }) {
  ...
  useEffect(() => {
    ...
    const particles = Array.from({ length: count }, () => ({
      ...
      r: Math.random() * 1.5 + 0.5, dx: (Math.random() - 0.5) * 0.3, dy: (Math.random() - 0.5) * 0.3, opacity: Math.random() * 0.4 + 0.1,
    }));
    const draw = () => {
...
export default function Signup() {
  ...
  // After email signup form, go to OTP
  ...
  // After Google company step, go to OTP
  const handleGoogleCompanySubmit = (company: string, wpp: string) => {
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-10 bg-background overflow-y-auto">
        <div className="w-full max-w-[380px] py-4">
          ...
          {step === "google-company" && (
            <GoogleCompanyStep
              userName={fullName}
              onSubmit={handleGoogleCompanySubmit}
            />
          )}
